...
inline std::ostream &operator<<(std::ostream &os, mapping_value const &v) { return os << queneromq::to_hex({reinterpret_cast<char const *>(v.buffer.data()), v.len}); }
...
struct name_system_db
{
  ...
  bool                        save_mapping   (crypto::hash const &tx_hash, cryptonote::tx_extra_quenero_name_system const &src, uint64_t height, int64_t owner_id, int64_t backup_owner_id = 0);
  ...
  // entry: (optional) if function returns true, the Quenero Name System entry in the TX extra is copied into 'entry'
  bool                        validate_lns_tx       (uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_quenero_name_system *entry = nullptr, std::string *reason = nullptr) const;
