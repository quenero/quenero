...
*Setup instructions for a Gitian build of Quenero.*

Gitian is the deterministic build process that is used to build the Quenero CLI
...
```bash
cp quenero/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```bash
./gitian-build.py --setup quenero-user x.x.x
```

Where `quenero-user` is your Github name and `x.x.x` is the version tag you want to build.
If you are using docker, run it with:

```bash
./gitian-build.py --setup --docker quenero-user v3.0.4
```
...
```bash
git clone git@github.com:quenero/gitian.sigs.git
git remote add quenero-user git@github.com:quenero-user/gitian.sigs.git
```
...
```bash
./gitian-build.py --detach-sign --no-commit -b quenero-user v3.0.4
```
...
```bash
export NAME=quenero-user
export VERSION=v3.0.4
gpg --output $VERSION-linux/$NAME/quenero-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/quenero-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/quenero-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/quenero-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/quenero-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/quenero-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[quenero/gitian.sigs](https://github.com/quenero/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/quenero-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/quenero-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/quenero-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/quenero/quenero -b <branch> <commit hash>
```
