...
#define QUENERO_DEFAULT_LOG_CATEGORY "pulse"
...
template <typename T>
using quorum_array = std::array<T, masternodes::PULSE_QUORUM_NUM_VALIDATORS>;
...
struct round_context
{
  ...
  struct
  {
    ...
    masternodes::quorum quorum;               // The block producer/validator participating in the next round
...
bool msg_signature_check(pulse::message const &msg, crypto::hash const &top_block_hash, masternodes::quorum const &quorum, std::string *error)
{
  std::stringstream stream;
  QUENERO_DEFER {
...
void relay_validator_handshake_bit_or_bitset(round_context const &context, void *quorumnet_state, masternodes::masternode_keys const &key, bool sending_bitset)
{
...
bool enforce_validator_participation_and_timeouts(round_context const &context,
                                                  ...
                                                  masternodes::masternode_list &node_list,
                                                  bool timed_out,
                                                  bool all_received)
{
...
void pulse::handle_message(void *quorumnet_state, pulse::message const &msg)
{
  ...
  // TODO(quenero): We don't support messages from future rounds. A round
  ...
  if (msg.quorum_position >= masternodes::PULSE_QUORUM_NUM_VALIDATORS)
  {
  ...
  switch(msg.type)
  {
    ...
    case pulse::message_type::signed_block:
    {
      ...
      // Masternode signatures. (Masternode signatures are added in one shot
      ...
      if (!crypto::check_signature(final_block_hash, validator_key, msg.signed_block.signature_of_final_block_hash))
      {
        MTRACE(log_prefix(context) << "Dropping " << msg_source_string(context, msg)
                                   ...
                                   << " does not validate with the Masternode");
        return;
      }
...
bool pulse::convert_time_to_round(pulse::time_point const &time, pulse::time_point const &r0_timestamp, uint8_t *round)
{
  ...
  size_t result_usize                 = time_since_round_started / masternodes::PULSE_ROUND_TIME;
...
bool pulse::get_round_timings(cryptonote::Blockchain const &blockchain, uint64_t block_height, uint64_t prev_timestamp, pulse::timings &times)
{
  ...
  times.r0_timestamp    = std::clamp(times.ideal_timestamp,
                                  times.prev_timestamp + masternodes::PULSE_MIN_TARGET_BLOCK_TIME,
                                  times.prev_timestamp + masternodes::PULSE_MAX_TARGET_BLOCK_TIME);
#else // NOTE: Debug, make next block start relatively soon
  times.r0_timestamp = times.prev_timestamp + masternodes::PULSE_ROUND_TIME;
#endif

  times.miner_fallback_timestamp = times.r0_timestamp + (masternodes::PULSE_ROUND_TIME * 255);
  return true;
}
...
Yes +-----[Block can not be added to blockchain]
  ...
  Wait For Next Block:
    ...
    - // TODO(quenero): After the Genesis Pulse Block is checkpointed, we can
  ...
  Prepare For Round:
    ...
    - The state machine *always* reverts to 'Prepare For Round' when any
      ...
      because of an insufficient Masternode network.
...
{
  if (masternodes::verify_pulse_quorum_sizes(context.prepare_for_round.quorum))
  {
...
round_state prepare_for_round(round_context &context, masternodes::masternode_keys const &key, cryptonote::Blockchain const &blockchain)
{
  //
  ...
  //
  {
    ...
    size_t round_usize           = time_since_block / masternodes::PULSE_ROUND_TIME;
  ...
  std::vector<crypto::hash> const entropy = masternodes::get_pulse_entropy_for_next_block(blockchain.get_db(), context.wait_for_next_block.top_hash, context.prepare_for_round.round);
  auto const active_node_list             = blockchain.get_masternode_list().active_masternodes_infos();
  uint8_t const hf_version                = blockchain.get_current_hard_fork_version();
  crypto::public_key const &block_leader  = blockchain.get_masternode_list().get_block_leader().key;

  context.prepare_for_round.quorum =
      masternodes::generate_pulse_quorum(blockchain.nettype(),
                                           block_leader,
                                           hf_version,
                                           active_node_list,
                                           entropy,
  ...
  if (!masternodes::verify_pulse_quorum_sizes(context.prepare_for_round.quorum))
  {
    MINFO(log_prefix(context) << "Insufficient Masternodes to execute Pulse on height " << context.wait_for_next_block.height << ", we require a PoW miner block. Sleeping until next block.");
    return goto_wait_for_next_block_and_clear_round_data(context);
  }
...
round_state send_and_wait_for_handshakes(round_context &context, void *quorumnet_state, masternodes::masternode_keys const &key)
{
  //
  // NOTE: Send
  //
...
round_state send_handshake_bitsets(round_context &context, void *quorumnet_state, masternodes::masternode_keys const &key)
{
  try
  {
...
round_state wait_for_handshake_bitsets(round_context &context, masternodes::masternode_list &node_list, void *quorumnet_state, masternodes::masternode_keys const &key)
{
  ...
  if (timed_out || all_bitsets)
  {
    ...
    if (count < masternodes::PULSE_BLOCK_REQUIRED_SIGNATURES || best_bitset == 0 || i_am_not_participating)
    {
      if (best_bitset == 0)
      {
      ...
      else
      {
        ...
        MDEBUG(log_prefix(context) << "We heard back from less than " << masternodes::PULSE_BLOCK_REQUIRED_SIGNATURES << " of the validators ("
...
round_state send_block_template(round_context &context, void *quorumnet_state, masternodes::masternode_keys const &key, cryptonote::Blockchain &blockchain)
{
  assert(context.prepare_for_round.participant == sn_type::producer);
  std::vector<masternodes::masternode_pubkey_info> list_state = blockchain.get_masternode_list().get_masternode_list_state({key.pub});

  // Invariants
  if (list_state.empty())
  {
    MWARNING(log_prefix(context) << "Block producer (us) is not available on the masternode list, waiting until next round");
    return goto_preparing_for_next_round(context);
  }

  std::shared_ptr<const masternodes::masternode_info> info = list_state[0].info;
  if (!info->is_active())
  {
    MWARNING(log_prefix(context) << "Block producer (us) is not an active masternode, waiting until next round");
    return goto_preparing_for_next_round(context);
  }
  ...
  {
    uint64_t height                              = 0;
    masternodes::payout block_producer_payouts = masternodes::masternode_info_to_payout(key.pub, *info);
...
round_state wait_for_block_template(round_context &context, masternodes::masternode_list &node_list, void *quorumnet_state, masternodes::masternode_keys const &key, cryptonote::Blockchain &blockchain)
{
...
round_state send_and_wait_for_random_value_hashes(round_context &context, masternodes::masternode_list &node_list, void *quorumnet_state, masternodes::masternode_keys const &key)
{
  assert(context.prepare_for_round.participant == sn_type::validator);
...
round_state send_and_wait_for_random_value(round_context &context, masternodes::masternode_list &node_list, void *quorumnet_state, masternodes::masternode_keys const &key)
{
  //
  // NOTE: Send
  //
  ...
  if (timed_out || all_values)
  {
    ...
    crypto::hash final_hash = {};
    {
      ...
      for (size_t index = 0; index < quorum.size(); index++)
      {
        if (auto &random_value = quorum[index]; random_value)
        {
          ...
          // trying to derive the Masternode rng seed.
    ...
    // Masternode signatures because we allow the first
    ...
    // valid against the quorum Masternodes).
...
round_state send_and_wait_for_signed_blocks(round_context &context, masternodes::masternode_list &node_list, void *quorumnet_state, masternodes::masternode_keys const &key, cryptonote::core &core)
{
  assert(context.prepare_for_round.participant == sn_type::validator);
  ...
  if (timed_out || enough)
  {
    ...
    std::array<size_t, masternodes::PULSE_QUORUM_NUM_VALIDATORS> indices = {};
    ...
    assert(indices_count >= masternodes::PULSE_BLOCK_REQUIRED_SIGNATURES);
    std::array<size_t, masternodes::PULSE_BLOCK_REQUIRED_SIGNATURES> selected = {};
    std::sample(indices.begin(), indices.begin() + indices_count, selected.begin(), selected.size(), tools::rng);

    // Add Signatures
    cryptonote::block &final_block = context.transient.signed_block.final_block;
    for (size_t index = 0; index < masternodes::PULSE_BLOCK_REQUIRED_SIGNATURES; index++)
    {
...
void pulse::main(void *quorumnet_state, cryptonote::core &core)
{
  ...
  masternodes::masternode_keys const &key = core.get_service_keys();
  ...
  masternodes::masternode_list &node_list = core.get_masternode_list();
