...
local debian_pipeline(name, image,
        ...
        werror=false, // FIXME
        build_tests=true,
        test_quenerod=true, # Simple quenerod offline startup test
        ...
        submodules,
        {
            ...
            commands: [
                ...
                'eatmydata ' + apt_get_quiet + ' install -y --no-install-recommends cmake git ca-certificates ninja-build ccache '
                    + deps + (if test_quenerod then ' gdb' else ''),
                'mkdir build',
                'cd build',
                ...
                if test_quenerod then [
                    '(sleep 3; echo "status\ndiff\nexit") | TERM=xterm ../utils/build_scripts/drone-gdb.sh ./bin/quenerod --offline --data-dir=startuptest'
                ] else []
            ) + (
                if run_tests then [
                    'mkdir -v -p $$HOME/.quenero',
...
local gui_wallet_step(image, wine=false) = {
    ...
    commands: [
        ...
        'cp -v build/bin/quenerod' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
        'cp -v build/bin/quenero-wallet-rpc' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
...
local gui_wallet_step_darwin = {
    ...
    commands: [
        'git clone https://github.com/loki-project/loki-electron-gui-wallet.git',
        'cp -v build/bin/{quenerod,quenero-wallet-rpc} loki-electron-gui-wallet/bin',
        'cd loki-electron-gui-wallet',
        'sed -i -e \'s/^\\\\( *"version": ".*\\\\)",/\\\\1-${DRONE_COMMIT_SHA:0:8}",/\' package.json',
        'npm install',
        'npm run build',
...
[
    ...
    debian_pipeline("Static (win64)", "ubuntu:focal", deps='g++ g++-mingw-w64-x86-64 '+static_build_deps,
                    ...
                    build_tests=false, lto=false, test_quenerod=false, extra_cmds=[
