...
from masternode_network import coins, vprint

def test_init(net):
    """Tests that the masternode test network got initialized properly.  (This isn't really a test
    ...
    active_sns = net.nodes[0].json_rpc("get_n_masternodes", dict(
        fields=dict(quorumnet_port=True, pubkey_ed25519=True),
        active_only=True)
        ).json()['result']['masternode_states']
