#ifndef QUENERO_NAME_SYSTEM_H
#define QUENERO_NAME_SYSTEM_H
...
{
  static size_t constexpr BUFFER_SIZE = std::max({WALLET_ACCOUNT_BINARY_LENGTH_INC_PAYMENT_ID,  SESSION_PUBLIC_KEY_BINARY_LENGTH}) + SODIUM_ENCRYPTION_EXTRA_BYTES;
  ...
  //                   , 52 character base32z string of an ed25519 public key
...
// not return the dedicated length types such  as those are only
...
/// type.  In particularly this maps all mapping_type:: values to the underlying value
/// of mapp.
...
// Currently accepts "session" or for lookups, buys, updates, and renewals; for buys and renewals also accepts for N=2,5,10
...
struct name_system_db
{
  ...
  bool                        save_mapping   (crypto::hash const &tx_hash, cryptonote::tx_extra_quenero_name_system const &src, uint64_t height, std::optional<uint64_t> expiration_height, int64_t owner_id, std::optional<int64_t> backup_owner_id);
  ...
  bool validate_ons_tx(uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_quenero_name_system &entry, std::string *reason);
